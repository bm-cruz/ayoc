...
alternate_sum_8:
  ...
  add EDI, R8D  ; + x5
  sub EDI, R9D  ; - x6
  add EDI, DWORD [RBP + 16]  ; + x7
  sub EDI, DWORD [RBP + 24]  ; + x8
...
product_2_f:
  ...
  mov ECX, ECX
  ret
  ; interpreto x1 como int64 para poder convertirlo a float
  ; hacer "mov ESI, ESI" es innecesario, los 32 bits mas altos ya estan en 0
  ;cvtsi2ss XMM1, RSI   ; x1 es float32
  ;mulss XMM1, XMM0     ; x1*f1 es float32
  ;cvttss2si RAX, XMM1  ; x1*f1 es int64
  ;mov EAX, EAX         ;
  ;mov [RDI], EAX       ; *destination = x1*f1
  ;ret
